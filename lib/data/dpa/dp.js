 ...
 * @param{object} obj - object containing pdf and cdf of weigth distribution
 ...
 *  weight distribution
 */
function wRandom(obj, replacement, keys) {
    // there is no 'real' topology, all objects are stored in a same directory
    if (obj.leaf) {
        return [''];
    }
    const len = keys.length;
    // not-found case
    if (!replacement && obj.pdf.length < len) {
        return undefined;
    }
